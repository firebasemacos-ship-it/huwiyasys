...
import { LayoutDashboard, ShoppingBag, Users, CreditCard, Ticket, LogOut, Settings, Image as ImageIcon, Box } from 'lucide-react';
    ...
    <div className="min-h-screen bg-muted/40" dir="rtl">
       <aside className="fixed inset-y-0 right-0 z-10 hidden w-64 flex-col border-l bg-background sm:flex">
        ...
        <nav className="flex-1 space-y-1 p-4">
            <Link href="/admin" className="flex items-center gap-3 rounded-lg bg-muted px-3 py-2 text-primary transition-all hover:text-primary">
      ...
      <main className="flex flex-1 flex-col gap-4 p-4 sm:mr-64 lg:gap-6 lg:p-6">
        <header className="flex h-16 items-center justify-between sm:justify-end">
            <Link href="/admin" className="flex items-center gap-2 font-semibold sm:hidden">
                <LayoutDashboard className="h-6 w-6" />
                <span className="text-lg font-semibold md:text-2xl">لوحة التحكم</span>
            </Link>
        </header>
